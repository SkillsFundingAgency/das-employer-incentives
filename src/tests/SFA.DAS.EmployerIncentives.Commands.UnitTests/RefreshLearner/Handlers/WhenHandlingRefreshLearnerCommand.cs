...
namespace SFA.DAS.EmployerIncentives.Commands.UnitTests.RefreshLearner.Handlers
{
    public class WhenHandlingRefreshLearnerCommand
    {
        ...
        public async Task Then_the_learning_not_found_reason_is_logged() // EI-490
        {
            ...
            //Assert
            _mockLogger.VerifyLogContains(LogLevel.Debug, Times.Once(), $"Start Learner data refresh from Learner match service for ApprenticeshipIncentiveId: {_learner.ApprenticeshipIncentiveId}, ApprenticeshipId: {_learner.ApprenticeshipId}, UKPRN: {_learner.Ukprn}, ULN: {_learner.UniqueLearnerNumber}");
            _mockLogger.VerifyLogContains(LogLevel.Debug, Times.Once(), $"End Learner data refresh from Learner match service for ApprenticeshipIncentiveId: {_learner.ApprenticeshipIncentiveId}, ApprenticeshipId: {_learner.ApprenticeshipId}, UKPRN: {_learner.Ukprn}, ULN: {_learner.UniqueLearnerNumber}");
        ...
        [Test]
        public async Task Then_LearningPeriodsChanged_flag_is_set_to_True_when_periods_have_changed()
        {
            //Arrange
            var command = new RefreshLearnerCommand(_apprenticeshipIncentiveId);
        ...
        [Test]
        public async Task Then_LearningPeriodsChanged_flag_is_set_to_False_when_periods_have_not_changed()
        {
            //Arrange
            var command = new RefreshLearnerCommand(_apprenticeshipIncentiveId);
            ...
            //Assert
            _mockLearnerDomainRepository.Verify(m => m.Save(
                It.Is<Learner>(l => l.GetModel().SubmissionData.LearningData.LearningPeriodsChanged == false)
                ), Times.Once);
        }
    }
}
