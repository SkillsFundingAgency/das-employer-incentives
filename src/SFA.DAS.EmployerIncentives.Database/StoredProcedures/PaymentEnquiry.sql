...
AS
	...
	left join (select AccountLegalEntityId, case when vrfvendorid is not null then 1 else 0 end as HasBank, VrfCaseStatus from [dbo].[Accounts] a) q on ai.AccountLegalEntityId = q.AccountLegalEntityId
	...
	group by Uln,[SubmissionFound], [LearningFound], [HasDataLock], [InLearning], q.HasBank, PausedPayments, DaysInLearning, q.VrfCaseStatus, l.UpdatedDate, pp.DueDate,pp.PeriodNumber,pp.PaymentYear, p.PaymentPeriod,p.PaymentYear, p.PaidDate, VrfVendorId, HashedLegalEntityId
