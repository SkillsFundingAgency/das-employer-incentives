...
namespace SFA.DAS.EmployerIncentives.Functions.PaymentProcess.UnitTests
{
    public class WhenRunningPaymentsProcess
    {
        ...
        public void Setup()
        {
            ...
            _mockOrchestrationContext.Setup(x => x.CallActivityAsync<List<PayableLegalEntity>>(nameof(GetPayableLegalEntities), _collectionPeriod, It.IsAny<TaskOptions>())).ReturnsAsync(_payableLegalEntities);            
            _mockOrchestrationContext.Setup(x => x.CallActivityAsync<List<ClawbackLegalEntity>>(nameof(GetUnsentClawbacks), _collectionPeriod, It.IsAny<TaskOptions>())).ReturnsAsync(_clawbackLegalEntities);
        ...
        public async Task Then_query_is_called_to_get_payable_legal_entities()
        {
            ...
            // assert
            _mockOrchestrationContext.Verify(x => x.CallActivityAsync<List<PayableLegalEntity>>("GetPayableLegalEntities", _collectionPeriod, It.IsAny<TaskOptions>()), Times.Once);
        }
        ...
        public async Task Then_process_pauses_after_generating_payments()
        {
            ...
            _mockOrchestrationContext.Verify(x => x.WaitForExternalEvent<bool>("PaymentsApproved", It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        {
            // arrange
            _mockOrchestrationContext.Setup(x => x.WaitForExternalEvent<bool>("PaymentsApproved", It.IsAny<CancellationToken>())).ReturnsAsync(false);

            // act
            await _orchestrator.RunOrchestrator(_mockOrchestrationContext.Object);

            // assert
            _mockOrchestrationContext.Verify(x => x.CallActivityAsync("SendPaymentRequestsForAccountLegalEntity", It.IsAny<AccountLegalEntityCollectionPeriod>(), It.IsAny<TaskOptions>()), Times.Never);
        }
        ...
        {
            // arrange
            _mockOrchestrationContext.Setup(x => x.WaitForExternalEvent<bool>("PaymentsApproved", It.IsAny<CancellationToken>())).ReturnsAsync(true);
            ...
            _mockOrchestrationContext.Verify(x => x.CallSubOrchestratorAsync("SendPaymentsForAccountLegalEntityOrchestrator", It.IsAny<AccountLegalEntityCollectionPeriod>(), It.IsAny<TaskOptions>()), Times.Exactly(expectedCallCount));
        ...
        public async Task Then_call_is_made_to_get_unsent_clawback_legal_entities()
        {
            ...
            // assert
            _mockOrchestrationContext.Verify(x => x.CallActivityAsync<List<ClawbackLegalEntity>>(nameof(GetUnsentClawbacks), _collectionPeriod, It.IsAny<TaskOptions>()), Times.Once);
        }
        ...
        public async Task Then_unsent_clawback_legal_entities_are_logged()
        {
            ...
            _mockOrchestrationContext
                .Setup(m => m.CallActivityAsync<List<ClawbackLegalEntity>>(nameof(GetUnsentClawbacks), _collectionPeriod, It.IsAny<TaskOptions>()))
