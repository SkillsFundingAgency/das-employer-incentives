        ...
        public SlackMessage Message => new SlackMessageBuilder()
                .AddSection(
                    new SlackSectionBuilder(block_id: nameof(MetricsReportGenerated))
                        .AddText(new SlackTextBuilder(SlackTextBuilder.Type.Plain, $"Payment approval email resent to  {_approverEmailAddress} for R{_collectionPeriod.PeriodNumber.ToString().PadLeft(2, '0')}.")
                     ))
                .AddSection(
